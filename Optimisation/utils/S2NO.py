...
"""
This version of S2NO is consistent with the framework in the "Modelling" folder, 
but the names of the modules are not the same.
"""
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.blocks = nn.ModuleList([attention_block(
...
class attention_block(nn.Module):
    ...
    def forward(self, fx):
        fx = self.Attn(self.ln_1(fx), self.lbo_bases, self.lbo_inver) + fx
...
class Attention_layer(nn.Module):
    def __init__(self, 
        ...
        if model_type == 'S2NO':
            ...
            self.LocalFeature = Graph_feature(num_channels = num_channels,  
                                              edge_index = edge_index)
